...
class Connection {
    constructor(extPart, name, messageHandler) {
        Utils.constructorTweakMethods('Connection', this);

        this.extPart = extPart;
        this.name = name;

        this._init(this.extPart, this.name, messageHandler);

        return {
            extPart: this.extPart,
            name: this.name,
    ...
    _init(extPart, name, messageHandler) {
        this._port = null;
        this._inited = false;
    ...
    _prepareMessage(message, cbPromiseResolve) {
    ...
    _postMessage(port, message, cbPromiseResolve) {
    ...
    _postResponse(fromPort, responseValue, origMessage) {
        let response = {
            from: this._myExtPart,
            to: origMessage.from,
    ...
    _handleMessage(message, fromPort) {
    ...
    _sendMessage(port, toExtPart, toNames, toTabId, userMessage, cbPromiseResolve) {
    ...
    _addMessengerPortNamePrefix(toNames) {
    ...
    _validateMessage(toExtPart, toName, toTabId) {
        if (!toExtPart) {
